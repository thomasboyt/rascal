...
describe('convertPiecesToSplineSegments', () => {
  ...
  it('rotates pieces relative to their entry', () => {
    ...
    expect(curve2.getPointAt(0)).toEqualVector3(new Vector3(0, 0, 0));
    expect(curve2.getPointAt(1)).toEqualVector3(new Vector3(1, 0, -1));

    const curve3 = result[2].curve;
    expect(curve3.getPointAt(0)).toEqualVector3(new Vector3(0, 0, 0));
    expect(curve3.getPointAt(1)).toEqualVector3(new Vector3(1, 0, -1));

    const curve4 = result[3].curve;
    expect(curve4.getPointAt(0)).toEqualVector3(new Vector3(0, 0, 0));
    expect(curve4.getPointAt(1)).toEqualVector3(new Vector3(0, 0, -2));
  });
});
